...
- **What's hardcoded**: 이 모양은 `AlgorithmExtractor`/`ExprParser`가 지금 인식하는 두 closure 관용구(`"the following steps[, given argument(s) ...]: ..."` / `"a [=term=] which performs the following steps when called with arguments ...: ..."`) 어디에도 안 맞아서, 그냥 통짜 문자열(`Expr.Str`)로 파싱돼버리고, 나중에 `CreateBuiltinFunction`이 그 문자열을 실제 코드인 것처럼 저장했다가 호출 시점에 `NoCallable`로 죽습니다. 이 패치는 소스 텍스트를 이미 지원되는 `"the following steps given argument |argValues|: ..."` 형태로 재작성해서 이 문제를 피해갑니다(`ExpandFollowingStepsPass`/`AddBuiltinBehaviourPass`가 그대로 hoist/3-arg builtin 변환을 해줌).
