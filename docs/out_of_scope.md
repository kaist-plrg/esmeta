...
- **근본 원인**: `personal/TODO.md` #33 — `spectec`의 `WasmHost`(`spectec/spectec/src/backend-interpreter/host.ml`)가 linear memory를 **바이트 하나당 JSON 객체 하나**로 RPC에 실어보냄. `new WebAssembly.Memory({initial:64, maximum:128})`(64페이지 = 4MB)만 돼도 circe가 이 JSON을 파싱/디코딩하다가 `-Xmx3g`(`.jvmopts`/`build.sbt`에 이미 설정된 값)로도 못 버티고 `OutOfMemoryError`를 던짐. 이 문서 #1(customSections)/#3(limits)과 달리 이건 SpecTec 서브모듈(RPC 브릿지 자체)을 고쳐야 하는 문제라 WJI 쪽에서 당장 근본 해결은 어려움 — TODO #33에 상세.
