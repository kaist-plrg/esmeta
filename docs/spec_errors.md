...
- **Expected**: `1. Let [|parameters|] → [|results|] be [=expand=](|functype|).`; `1. Let [|types|] → [] be [=expand=]([=tag_type=](|store|, ...)).`
...
- **Expected**: `1. Let (|store|, |tagAddress|) be [=tag_alloc=](|store|, [=fold=](|wasmParameters| → « »)).` and `1. Let (|store|, |tagAddress|) be [=tag_alloc=](|store|, [=fold=](« [=externref=] » → « »)).`
