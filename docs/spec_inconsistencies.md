...
- **WJI 쪽 처리**: `SpecPatch` #52로 우회 — dfn 문장 앞에 `<div algorithm>\n`, 마지막 스텝 뒤에 `\n</div>`를 각각 삽입(두 개의 짧은 삽입, 블록 전체를 감싸는 하나의 큰 치환 대신). 추출 자체는 성공하지만, 처음엔 컴파일된 함수 이름이 `INTRINSICS.get:WebAssembly.WebAssembly.prototype.JSTag`(중복 + 잘못된 `.prototype.`)로 나왔음 — `JSTag`가 인터페이스가 아니라 네임스페이스 자체에 붙는 getter라는 걸 컴파일러/lowering 쪽이 구분 못 했던 것. `AlgorithmKind.NamespaceGetter`(`interface.any.js`의 `compile`/`validate`/`instantiate`를 위해 추가한 `NamespaceMethod`의 getter 대응)로 해결됨 — `exception/jsTag.tentative.any.js` 3/3 완전 통과.
