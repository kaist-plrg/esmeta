...
Figure~\ref{fig:add-eval-algo} shows the \esalg{Evaluation} algorithm of the
...
\esalg{Call} algorithm in ECMA-262 represents function calls.  Thus, if we want
to know whether the addition operator can cause implicit function calls, we
should check that it is reachable from the \esalg{Evaluation} algorithm of the
...
(ES12, 2021) and demonstrate its practicality with \inred{three} advanced
syntactic views.
