...
// WebIDL conversion boundary (ICallConvert)
case class UnknownConversion(fname: String)
  extends InterpreterError(s"unknown WebIDL conversion function: $fname")
case class NoMathValue(v: Value)
  extends InterpreterError(s"cannot convert $v to a mathematical value")
