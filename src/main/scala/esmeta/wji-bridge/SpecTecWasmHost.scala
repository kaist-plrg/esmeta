...
/** [[WasmHost]] implementation backed by the SpecTec Wasm interpreter,
  ...
  * [[ALValue]]/[[Json]] `params`, call `connection.request("<embedding
  * function name>", params)`, and decode the `result`/`error` into the
  * `WasmHost` return types. SpecTec owns validation (unknown function,
  * arg type/arity mismatches); this class only maps SpecTec-side errors to
  * [[WasmError]].
  *
  ...
  * [[hostFunctions]] by id, runs the [[HostFunction]], and encodes its
  * result back to SpecTec — this is how reentrant Wasm -> wjmeta calls are
  * served.
  */
final class SpecTecWasmHost(connection: JsonRpcConnection) extends WasmHost:
  ...
  /** registry of [[HostFunction]]s passed to [[funcAlloc]], keyed by the id
    ...
    * below.
    */
  private val hostFunctions = scala.collection.concurrent.TrieMap.empty[String, HostFunction]
  ...
  connection.onRequest {
    case ("host_func_invoke", params) =>
      val decoded =
        ...
        yield (id, args)
      decoded match
        case Left(e) => Left(RpcError(-32602, s"invalid host_func_invoke params: ${e.getMessage}", None))
        case Right((id, args)) =>
          hostFunctions.get(id) match
            case None => Left(RpcError(-32602, s"unknown host function id: $id", None))
            case Some(hostFunc) =>
              hostFunc(args) match
                case Right(results) =>
                  val out: ALValue = ALValue.ListV(results)
                  Right(out.asJson)
                case Left(WasmError.Trap(value))  => Left(RpcError(-32000, "host function trap", Some(value.asJson)))
                case Left(WasmError.ProtocolError(msg)) => Left(RpcError(-32603, msg, None))
    case (other, _) =>
  ...
  /** Send `method`/`params` to SpecTec and decode the single-[[ALValue]]
    * `result`, mapping transport failures / SpecTec errors to [[WasmError]].
    * A SpecTec `error` whose `data` is an AL value becomes [[WasmError.Trap]].
    */
  private def call(method: String, params: Json): Either[WasmError, ALValue] =
    connection.request(method, params) match
      ...
      case Right(Response(_, Some(result), _, _)) =>
        result.as[ALValue].left.map(e => WasmError.ProtocolError(s"bad result: $e"))
      ...
      case Right(_) => Left(WasmError.ProtocolError("response had neither result nor error"))
