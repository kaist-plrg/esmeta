...
/** A wjmeta-side (WJI mechanization) closure that SpecTec calls back into
  * while executing Wasm code, corresponding to the `hostfunc` argument of
  ...
  *
  * `wjmeta-mechanize` provides implementations (running the mechanized WJI
  * IR interpreter); `wjmeta-bridge` registers them with [[WasmHost.funcAlloc]]
  * and dispatches incoming `host_func_invoke` requests from SpecTec to them.
  */
