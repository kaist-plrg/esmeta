...
/** Converts `AlgoCall`-with-args that appear in expression position into
  * explicit `Perform` statements so the compiler can emit `ICall` instead of
  * `EYet("call ...")`.
  *
  * Handles:
  * {{{
  ...
  * }}}
  *
  * Zero-argument `AlgoCall`s are left alone (the compiler emits
  * `ERef(Global(name))` for those, which is already correct).
  *
...
object ExtractInlineAlgoCallPass extends DesugarPass:
  ...
  /** Unwraps an optional `!` (assert-not-abrupt) wrapper and returns `(link,
    * args)` if the expression is an `AlgoCall`.
    *
    * @param zeroArg
    *   if true, zero-argument calls are included (for Let RHS); if false, only
    *   calls with arguments (for Return value).
    */
