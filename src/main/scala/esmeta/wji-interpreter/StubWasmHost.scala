...
class StubWasmHost extends WasmHost:
  ...
  def moduleInstantiate(store: ALValue, module: ALValue, externVals: List[ALValue]) =
  ...
  def funcType(store: ALValue, funcAddr: ALValue) = stub("func_type", store, funcAddr)
  ...
  def tableType(store: ALValue, tableAddr: ALValue) = stub("table_type", store, tableAddr)
  ...
  def tableSize(store: ALValue, tableAddr: ALValue) = stub("table_size", store, tableAddr)
  def tableGrow(store: ALValue, tableAddr: ALValue, n: ALValue, ref: ALValue) =
    stub("table_grow", store, tableAddr, n, ref)

  def memAlloc(store: ALValue, memType: ALValue) = stub("mem_alloc", store, memType)
  def memType(store: ALValue, memAddr: ALValue) = stub("mem_type", store, memAddr)
  def memSize(store: ALValue, memAddr: ALValue) = stub("mem_size", store, memAddr)
  def memGrow(store: ALValue, memAddr: ALValue, n: ALValue) = stub("mem_grow", store, memAddr, n)

  def tagAlloc(store: ALValue, tagType: ALValue) = stub("tag_alloc", store, tagType)
  def tagType(store: ALValue, tagAddr: ALValue) = stub("tag_type", store, tagAddr)
  ...
  def exnRead(store: ALValue, exnAddr: ALValue) = stub("exn_read", store, exnAddr)

  def globalAlloc(store: ALValue, globalType: ALValue, value: ALValue) =
    stub("global_alloc", store, globalType, value)
  def globalType(store: ALValue, globalAddr: ALValue) = stub("global_type", store, globalAddr)
  def globalRead(store: ALValue, globalAddr: ALValue) = stub("global_read", store, globalAddr)
