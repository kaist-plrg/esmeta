...
object IrPrinter:
  ...
  def render(func: Func): String =
    val params = func.params.map(p => p.name.name + (if p.optional then "?" else "")).mkString(", ")
  ...
  def renderInst(inst: Inst, indent: Int): String =
    ...
    inst match
      ...
      case IAssign(ref, expr)          => s"${pad}${renderRef(ref)} = ${renderExpr(expr)}"
      case IDelete(ref)                => s"${pad}delete ${renderRef(ref)}"
      case IPush(elem, list, true)     => s"${pad}prepend ${renderExpr(elem)} to ${renderExpr(list)}"
      case IPush(elem, list, false)    => s"${pad}append ${renderExpr(elem)} to ${renderExpr(list)}"
      case IPop(lhs, list, true)       => s"${pad}let ${lhs.name} = pop front from ${renderExpr(list)}"
      case IPop(lhs, list, false)      => s"${pad}let ${lhs.name} = pop back from ${renderExpr(list)}"
  ...
  def renderExpr(expr: Expr): String = expr match
    ...
    case EBinary(op, l, r)    => s"(${renderExpr(l)} ${renderBOp(op)} ${renderExpr(r)})"
