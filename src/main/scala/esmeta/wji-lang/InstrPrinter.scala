...
object InstrPrinter:
  ...
  def render(algo: Algorithm): String =
    ...
    if algo.params.nonEmpty then sb.append(s"  params: ${algo.params.mkString(", ")}\n")
    if algo.head.nonEmpty then sb.append(s"  head: ${algo.head}\n")
    renderInstrs(sb, algo.body, indent = 1)
    sb.toString

  private def renderInstrs(sb: StringBuilder, instrs: List[Instr], indent: Int): Unit =
    for instr <- instrs do
  ...
  private def renderCall(func: String, args: List[Expr]): String =
    if args.isEmpty then func else s"$func(${args.map(ExprPrinter.render).mkString(", ")})"

  def render(instr: Instr): String = instr match
    case Let(lhs, expr, _)                         => s"Let(${ExprPrinter.render(lhs)}, ${ExprPrinter.render(expr)})"
    case Set(lhs, expr, _)                         => s"Set(${ExprPrinter.render(lhs)}, ${ExprPrinter.render(expr)})"
    case If(cond, _)                               => s"If(${CondPrinter.render(cond)})"
    case ElseIf(cond, _)                           => s"ElseIf(${CondPrinter.render(cond)})"
    case Else(_)                                   => "Else"
    case Return(expr, _)                           => s"Return(${expr.map(ExprPrinter.render).getOrElse("")})"
    ...
    case Append(item, coll, _)                     => s"Append(${ExprPrinter.render(item)}, ${ExprPrinter.render(coll)})"
    case Continue(_)                               => "Continue"
    case Perform(func, args, Discard, _)           => s"Perform(${renderCall(func, args)})"
    case Perform(func, args, ReturnResult, _)      => s"PerformAndReturn(${renderCall(func, args)})"
    case Perform(func, args, BindResult(v), _)     => s"PerformAndLet(${renderCall(func, args)}, $v)"
