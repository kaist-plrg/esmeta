...
/** Bracket/quote-aware substring search and splitting, used to find
  * separators at the "top level" of spec prose — i.e. outside any
  * `(...)`, `[...]`, `{...}`, `«...»`, or `"..."`.
  */
private[wji] object TextSplit:
  ...
  /** the index (and matched separator) of the first top-level occurrence
    * of any of `seps` in `text`, if any
    */
  ...
  def splitTopLevel(text: String, sep: String): Option[(String, String)] =
    findTopLevel(text, sep).map(i => (text.substring(0, i), text.substring(i + sep.length)))
