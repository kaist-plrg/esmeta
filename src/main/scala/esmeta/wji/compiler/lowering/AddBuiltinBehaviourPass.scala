...
/** Adapts every closure [[ExpandFollowingStepsPass]] hoisted for
  * `CreateBuiltinFunction`'s `behaviour` argument into a valid builtin
  * function body. Two independent fix-ups `manuals/rule.json`'s hand-patched
  * `BuiltinCallOrConstruct` rule requires, neither of which
  * `ExpandFollowingStepsPass` itself is responsible for — that pass only
  * hoists a closure/`Algorithm` pair, with no notion of what convention the
  * closure is ultimately invoked under:
  *
  *   - '''Builtin arg unpacking''': `BuiltinCallOrConstruct` always invokes a
  *     builtin as `func.__CODE__(thisArgument, argumentsList, newTarget)` —
  *     a fixed 3-argument calling convention — regardless of what parameters
  *     the underlying closure itself declares. Mirrors mainline
  ...
  *     (e.g. `NewPromiseCapability`'s resolve/reject functions) by giving
  *     them the builtin signature plus a prefix that unpacks `argumentsList`
  *     positionally into the real parameter names.
  *   - '''Completion-record wrapping''': `manuals/rule.json`'s Call rule
  *     (`Let result be the Completion Record that is the result of
  *     evaluating... F.__CODE__(...)`) reads `.[[Type]]`/`.[[Value]]` off
  *     whatever `F.__CODE__` returns unconditionally, mirroring mainline's
  *     own `esmeta.compiler.Compiler.compile`'s `Builtin => needRetComp =
  *     true` — *not* gated on whether the closure itself can abruptly
  *     complete, unlike [[WrapCompletionReturnsPass]]'s own
  *     `returnsCompletion` test for ordinary algorithms. Reuses
  *     [[CompletionWrapping.expand]] directly — see that object's own doc for
  *     why it's a shared plain function rather than living inside
  ...
  * existed. Collected as a plain `Set[String]` of closure names local to
  * `run`, not persisted as an `Algorithm` field the way `returnsCompletion`
  * is: unlike that field, nothing else in the pipeline needs this fact, so a
  * separate mark-then-consume pair of passes would only add indirection here.
  *
...
object AddBuiltinBehaviourPass extends LoweringPass:
  ...
  /** formal parameter names `BuiltinCallOrConstruct`'s hand-patched IR always
    * calls `func.__CODE__` with, regardless of what parameters the
    * underlying `behaviour` closure itself declares.
    */
  ...
  private def collectBuiltinClosureNames(instrs: List[Instr]): Set[String] =
    instrs match
      ...
      case instr :: rest =>
        collectBuiltinClosureNames(instr.body) ++ collectBuiltinClosureNames(rest)
  ...
  /** the `params.zipWithIndex` prefix instructions that unpack a builtin's
    ...
    * mainline `Compiler.getBuiltinPrefix`'s `Param(name, _, Normal)` case
    * (a hoisted WJI closure never has a variadic/optional parameter, so
    * there's no need for the other cases that function has).
    */
  ...
  def run(algos: List[Algorithm]): List[Algorithm] =
    val builtinNames = algos.flatMap(a => collectBuiltinClosureNames(a.body)).toSet
    algos.map { a =>
      if a.name.exists(builtinNames.contains) then
        ...
        a.copy(
          ...
          body =
            unpackArgumentsList(originalParams) ++ CompletionWrapping.expand(a.body),
        )
      else a
    }
