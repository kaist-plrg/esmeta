...
object ExpandAbbreviatedCondPass extends LoweringPass:
  ...
  private def transformInstr(instr: Instr): Instr = instr match
    case i: Instr.If =>
      i.copy(cond = expandCond(i.cond), body = transformInstrs(i.body))
    case i: Instr.ElseIf =>
      i.copy(cond = expandCond(i.cond), body = transformInstrs(i.body))
    case i: Instr.While =>
      i.copy(cond = expandCond(i.cond), body = transformInstrs(i.body))
    ...
    case i: Instr.IfChain =>
      i.copy(
        branches =
          i.branches.map((c, b) => (expandCond(c), transformInstrs(b))),
  ...
  private def leftmostTemplate(cond: Cond): Option[Expr => Cond] = cond match
    ...
    case Cond.Matches(subj, mt, _, neg) =>
      Some(expr => Cond.Matches(subj, mt, expr, neg))
  ...
  /** Apply a template to every [[Cond.Abbreviated]] inside `cond`, while also
    ...
    */
  private def applyTemplate(cond: Cond, tmpl: Option[Expr => Cond]): Cond =
    cond match
