...
/** Rewrites every `Expr.FollowingSteps(params)` placeholder — "the following
  ...
  */
object ExpandAbstractClosurePass extends LoweringPass:
  private var counter = 0
  private def freshName(): String =
    counter += 1; s"_closure$counter"
  ...
  private def transform(
    ...
    case Instr.Let(lhs, Expr.FollowingSteps(params), body) :: rest
        if body.nonEmpty =>
      Instr.Let(lhs, hoist(params, body, extra), Nil) :: transform(rest, extra)
    case (p: Instr.Perform) :: rest
      ...
      val closure = hoist(params, p.body, extra)
      val newArgs = p.args.map {
        case Expr.FollowingSteps(_) => closure
        case other                  => other
      }
      p.copy(args = newArgs, body = Nil) :: transform(rest, extra)
    case instr :: rest =>
