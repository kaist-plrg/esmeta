...
/** Expands a tuple-destructuring `Let` immediately followed by an `If
  ...
  * (|store|, |memaddr|) be [=mem_alloc=](...). If allocation fails, ..." — with
  * no variable named in the check at all. This pass recovers the missing
  * subject from the immediately preceding `Let`'s own LHS tuple, checking its
  * *last* element (the embedding function's own "real" result —
  * `memaddr`/`tableaddr` — as opposed to `store`, which is present either way)
  * against `[=error=]`.
  *
  * {{{
  ...
  * this idiom is ever paired with in the corpus — can actually produce `error`
  * today (see `docs/hardcodes.md`: both defer to `$allocmem`/ `$alloctable`,
  * total functions in `.spectec`'s own formalization, so
...
object ExpandAllocationFailsPass extends LoweringPass:
  ...
  /** Must precede:
    *   - [[ExpandDestructuringLetPass]]: needs the preceding `Let`'s LHS still
    *     a raw `Expr.Tuple`, not yet expanded into a fresh `_tupleN` temp +
    *     per-element `TupleProj` extractions — `ExpandDestructuringLetPass`
