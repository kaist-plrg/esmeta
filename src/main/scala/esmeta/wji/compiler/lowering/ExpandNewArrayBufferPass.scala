...
/** Expands `Expr.NewArrayBuffer` — "a new {{ArrayBuffer}} with the internal
  * slots [[ArrayBufferData]], [[ArrayBufferByteLength]], ..." — into a
  * `JSCall` on ECMA-262's own `AllocateArrayBuffer(constructor, byteLength)`,
  * against the current realm's `%ArrayBuffer%` intrinsic:
  *
  * {{{
  ...
  * }}}
  *
  * `ArrayBuffer` isn't a js-api-defined interface (it's an ECMA-262
  * primitive js-api merely reuses), so this deliberately does *not* go
  * through `Expr.New`/WebIDL's platform-object construction — see
  * `docs/underspecified-behaviors.md` for why no algorithm is actually named
  * for this phrasing in the spec text itself. The placeholder `byteLength=0`
  * is fine: the spec's own very next steps
  * (`Set buffer.[[ArrayBufferData]] to block`,
  * `Set buffer.[[ArrayBufferByteLength]] to length(block)`) immediately
  ...
  * webidl/index.bs:9273's own `[=?=] [$AllocateArrayBuffer$](...)` call),
  * so its raw result is a Completion Record, not the `ArrayBuffer` object
  ...
  * `ExpandAbruptPass` unwrapping every other `?`/`!`-marked call already
  * gets, correctly binding `target` to the Completion Record's `.Value`
  * instead. `AllocateArrayBuffer` can only actually fail via a `RangeError`
  * for an oversized `byteLength`; ours is always the literal `0`, so `!`
  * (assert-not-abrupt) rather than `?` (propagate) is the correct marker —
  * there is no enclosing completion-returning context for `?` to propagate
  * into here anyway (`create a fixed length memory buffer`'s own body has no
  * `Throw`/abrupt path at all).
  *
  * Only handles `NewArrayBuffer` in direct `Let` RHS position — the only
  * shape observed in practice (`create a fixed length memory buffer`/
