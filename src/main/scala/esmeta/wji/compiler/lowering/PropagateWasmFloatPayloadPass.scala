...
/** Threads the bit width `ExpandIsOfFormPass.buildFormMatch` attaches to an
  ...
  * ...)`, bound to a plain `Var` by a `Let` right at the `IsOfForm`
  * destructure site — forward to wherever that `Var` is later read back out
  * through `Expr.AsMath` ("|f32|/|f64| interpreted as a [=mathematical
  * value=]", `ToJSValue`'s own phrasing), which is the one place the width
  * still matters (`Compiler`'s `AsMath(WasmFloatPayload(width, e))` case,
  * `docs/hardcodes.md` #19's counterpart entry).
  *
  ...
  * other) `CONST` payload; that fact only still exists at the `Let` that
  * bound it. This pass is a small single-pass forward propagation over each
  ...
  * `Compiler`'s special-cased nested match on exactly that shape is what
  * then picks the right conversion (`docs/hardcodes.md` #19's `ToMathF32`/
  ...
  * `Var` this pipeline produces is effectively assigned exactly once (no
  * reuse of the same name for an unrelated binding later in the same
  * algorithm), the same assumption every other single-pass rewrite in this
...
object PropagateWasmFloatPayloadPass extends LoweringPass:
  ...
  /** Requires:
    *   - [[ExpandIsOfFormPass]]: needs `Expr.WasmFloatPayload` already
    *     attached to the `Let` bindings it produces for an f32/f64 `CONST`
    *     payload's own `Var` — this pass does no destructuring of its own,
    *     only forward-propagates a shape that pass already built.
    */
