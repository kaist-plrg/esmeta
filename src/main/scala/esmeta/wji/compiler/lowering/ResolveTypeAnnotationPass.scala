...
/** Resolves every `Expr.TypeAnnotated` node `ExprParser` produces uniformly
  * for a `"the [=TERM=] EXPR"` (or `"the {{TERM}} value EXPR"`) prefix — see
  * that node's own doc for why `ExprParser` can't make this call itself.
  *
  * Drops TERM whenever EXPR already carries its own tag (anything other than
  * an untagged `Expr.Seq_`) — the common case, and exactly
  * `TypeAnnotatedPrefix`'s old parse-time behavior, restored here instead.
  * Leaves `TypeAnnotated` wrapping a `Seq_` untouched for now: assigning TERM
  * as the real SpecTec tag there is deferred to a future normalize pass
  * (mirrors `Seq_`'s own doc), not this one's job — this pass only resolves
  * the "drop or keep" ambiguity, not what to do with a kept one.
  *
