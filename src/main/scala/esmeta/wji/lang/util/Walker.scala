...
/** A structural rewriter for metalang ASTs, mirroring `esmeta.ir.util.Walker`:
  * a concrete pass extends this and overrides only the `walk` overload for
  * the node type(s) it actually transforms, calling `super.walk(...)` for
  * everything else. Every other node type recurses through its children
  * unchanged by default — a pass never needs to hand-write the full
  * exhaustive `Expr`/`Cond`/`Instr` case list itself just to reach whichever
  * one or two constructors it actually cares about, and dynamic dispatch
  * means an override on one node type is picked up wherever the default
  * recursion for any *other* node type reaches it (see `esmeta.ir.util.
  * YetCollector` for the same pattern already proven in mainline ESMeta).
  */
trait Walker:
