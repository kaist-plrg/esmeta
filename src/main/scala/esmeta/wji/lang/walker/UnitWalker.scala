...
trait UnitWalker:
  ...
  def walk(cond: Cond): Unit = cond match
    ...
    case Cond.Exposed(subject, realm, _) => walk(subject); walk(realm)
    case Cond.And(l, r)                  => walk(l); walk(r)
    case Cond.Or(l, r)                   => walk(l); walk(r)
    case Cond.Abbreviated(e)             => walk(e)
    case Cond.Any(_, collections, body, _) =>
  ...
  def walk(instr: Instr): Unit = instr match
    ...
    case i: Instr.Pop      => walk(i.lhs); walk(i.list); i.body.foreach(walk)
