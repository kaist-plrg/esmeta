...
trait ESMetaAsyncTest extends funsuite.AnyFunSuite with BeforeAndAfterAll {
  // set test mode
  TEST_MODE = true
  ...
  // check result
  def check[T](name: String)(tester: => T): Future[Unit] = {
    Future {
      this.synchronized { count += 1 }
      {
        try {
          tester
          this.synchronized { resMap += name -> Pass }
        } catch {
          case e @ NotSupported(msg) =>
            resMap += name -> Yet(msg)
          case e: Throwable =>
            println(e)
            this.synchronized { resMap += name -> Fail }
        }
      }
    }
  }
