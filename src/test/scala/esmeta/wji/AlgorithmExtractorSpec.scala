...
class AlgorithmExtractorSpec extends AnyFunSuite:
  ...
  test("reconstructs nested instrs (including bullet sub-lists) from indentation") {
  ...
  test("falls back to the algorithm=\"...\" attribute as id when there is no <dfn>") {
    val algo = algorithms.find(_.id.contains("js-string-cast")).get
    assert(algo.name.isEmpty)
    assert(algo.body == List(Return(Some(Abrupt("?", JSCall("UnwrapString", List(Var("v"))))))))
  }

  test("supports algorithms with no instrs at all") {
    val algo = algorithms.find(a => a.name.contains("exports") && a.params.isEmpty).get
    assert(algo.body.isEmpty)
    assert(algo.head.nonEmpty)
  }

  test("extracts params written inside the <dfn> as 'name(|arg1|, |arg2|)', stripping them from name") {
  ...
  test("extracts params that appear both right after '</dfn>' and later in the prose") {
    val algo = algorithms.find(_.name.contains("initialize an instance object")).get
    assert(algo.params == List("|instanceObject|", "|module|", "|instance|"))
  }

  test("supports algorithms with no params") {
    val algo = algorithms.find(_.name.contains("get the JavaScript exception tag")).get
