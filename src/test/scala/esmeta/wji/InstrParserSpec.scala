...
class InstrParserSpec extends AnyFunSuite:
  ...
  test("parses a flat sequence of Let/If/Return") {
    ...
    assert(
      instrs == List(
        Let(
          Var("first"),
          Abrupt("?", JSCall("UnwrapString", List(Var("first")))),
        ),
        Let(
          Var("second"),
          Abrupt("?", JSCall("UnwrapString", List(Var("second")))),
        ),
        If(
          Eq(
            Abrupt(
  ...
  test(
    "treats a ', and ...' clause as part of the condition, not the action separator",
  ) {
  ...
  test("parses 'For each ELEM of/in COLLECTION' with a nested body") {
    val algo = algorithms
      .find(_.id.contains("create the WebAssembly namespace object"))
      .get
    val instrs = algo.body
    assert(
      instrs(1) == ForEach(
        ...
        List(
          Let(
            Var("constructor"),
            Expr.UnknownNew(
              "a new object, implementing the [=NativeError Object Structure=], with <var ignore>NativeError</var> set to |error|",
            ),
          ),
          Perform(
            "[=!=]",
  ...
  test(
    "attaches nested sub-steps as the body of their introducing instr, even for Let",
  ) {
    val algo = algorithms.find(_.name.contains("create a host function")).get
    val instrs = algo.body
    val hostfunc = instrs.collectFirst {
      case l @ Let(Var("hostfunc"), _, _) => l
    }.get
    assert(
      hostfunc.body.head == Let(
        Var("realm"),
        Expr.Unknown("|func|'s [=associated Realm=]"),
      ),
    )
  }
  ...
  test(
    "parses 'Perform OP, and return the result' as Perform with ReturnResult",
  ) {
    val algo = algorithms
      .find(a => a.name.contains("instantiate") && a.params.contains("|bytes|"))
      .get
    val instrs = algo.body
    assert(
      instrs.contains(
        Perform(
          ...
          List(Var("promiseOfModule"), Var("importObject")),
          ReturnResult,
        ),
      ),
    )
  ...
  test(
    "parses 'Perform OP, and let |x| be the result' as Perform with BindResult",
  ) {
    val algo = algorithms.find(_.id.contains("read-the-imports")).get
    val instrs = algo.body
    assert(
  ...
  test(
    "parses 'N<sup>M</sup>' as Pow, '-N<sup>M</sup>' as Neg(Pow), and Pow combined with &minus; as BinOp",
  ) {
  ...
  test("parses '[=is [not] a[n] TYPE=]' as IsType") {
    ...
    assert(
      unwrap.body.head == If(
        IsType(Var("v"), "String", negated = true),
        List(
  ...
  test("parses 'the length of X' and 'the number of elements in X' as Length") {
    val memAlgo = algorithms
      .find(_.name.contains("create a fixed length memory buffer"))
      .get
    assert(
      containsDeep(
        memAlgo.body,
    ...
    val strAlgo =
      algorithms.find(_.id.contains("js-string-fromCharCodeArray")).get
    assert(containsDeep(strAlgo.body, Let(Var("length"), Length(Var("array")))))
  }

  test(
    "parses 'the value of the element stored at index I in A' as Index(A, I)",
  ) {
    val algo = algorithms.find(_.id.contains("js-string-fromCharCodeArray")).get
    assert(
      containsDeep(
        algo.body,
        Let(Var("charCode"), Index(Var("array"), Var("i"))),
      ),
    )
  }
